...
// Detectar base del proyecto dinámicamente (sirve para rutas absolutas)
$BASE = dirname(dirname($_SERVER['SCRIPT_NAME'])); // /miproyecto
?>
<!DOCTYPE html>
<html lang="es">
<head>
      ...
      <table>
        ...
        <tbody>
          <?php foreach ($productos as $p): ?>
            <?php
              // Si la imagen es URL externa, la uso tal cual. Si no, busco en assets/img/
              $src = (preg_match('#^https?://#', $p['imagen']))
                ? $p['imagen']
                : $BASE . '/assets/img/' . $p['imagen'];
            ?>
            <tr>
              <td><?= (int)$p['id'] ?></td>
